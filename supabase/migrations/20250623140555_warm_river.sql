/*
  # Добавление полей для конвертации валют в таблицу payment_addresses
  
  1. Новые поля
    - `exchange_rate` (numeric) - Курс обмена (сколько единиц криптовалюты в 1 рубле)
    - `name` (text) - Отображаемое имя метода платежа
  
  2. Обновления
    - Установка базовых курсов обмена для существующих записей
    - Установка отображаемых имен для методов оплаты
  
  3. Индексы
    - Создание индекса для ускорения поиска по типу платежа
    - Проверка и добавление уникального ограничения на поле type
*/
